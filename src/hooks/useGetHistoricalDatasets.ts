...
import { getReqquiredFetchStates } from "./useGetCurrentDatasets";
...
export function useGetHistoricalDatasets(currentSates: string[]) {
  ...
  useEffect(() => {
    fetchHistoricalDatasets(reqquiredFetchStates).then((requiredDatasets) => {
      if (datasets) {
        setDatasets([...datasets, ...(requiredDatasets as Record[][])]);
      } else {
...
function getDisplayedDatasets(
  ...
  for (let i = 0; i < datasets.length; i++) {
    ...
    if (currStates.includes(state as string)) {
